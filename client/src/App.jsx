                    ...
                    {data && (
                        <div className="results-section">
                            ...
                            <div className="tab-content" style={{ marginTop: '30px' }}>
                                ...
                                {activeTab === 'results' && (
                                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={sgpaGridStyle}>
                                        {[1, 2].filter(sem => data.results.some(r => r.semester === sem)).map(sem => {
                                            const semResults = data.results.filter(r => r.semester === sem);
                                            const totalSubjects = semResults.length;
                                            const passCount = semResults.filter(r => r.status === 'Pass').length;
                                            const failCount = semResults.filter(r => r.status === 'Fail').length;
                                            const semSgpa = data.sgpa.find(s => s.semester === sem)?.sgpa || 'N/A';
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div style={semesterCardHeaderStyle}>
                                                        ...
                                                        <span style={semesterCardTitleStyle}>Semester {sem}</span>
                                                    </div>
                                                    <div style={semesterCardStatsStyle}>
                                                        <div style={semesterCardStatItemStyle}>
                                                            <span style={semesterCardStatLabelStyle}>SGPA</span>
                                                            <span style={semesterCardStatValueStyle}>{semSgpa}</span>
                                                        </div>
                                                        <div style={semesterCardStatItemStyle}>
                                                            <span style={semesterCardStatLabelStyle}>Subjects</span>
                                                            <span style={semesterCardStatValueStyle}>{totalSubjects}</span>
                                                        </div>
                                                        <div style={semesterCardStatItemStyle}>
                                                            <span style={semesterCardStatLabelStyle}>Backlogs</span>
                                                            <span style={{
                                                                ...semesterCardStatValueStyle,
                                                                color: failCount > 0 ? 'var(--danger)' : 'var(--success)'
                                                            }}>
                                                                {failCount}
                                                            </span>
                                                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            <div style={resultsModalHeaderStyle}>
                                                <h3 style={{ margin: 0 }}>Semester {selectedSemesterResults} Results</h3>
                                                <button onClick={() => setSelectedSemesterResults(null)} style={resultsModalCloseBtnStyle}>✕</button>
                                            </div>
                                            ...
                                            <div className="table-container desktop-only-results">
                                                <table className="results-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                                                    ...
                                                    <tbody>
                                                        {data.results
                                                            .filter(r => r.semester === selectedSemesterResults)
                                                            .map((r, i) => (
                                                                <tr key={i}>
                                                                    ...
                                                                    <td style={{
                                                                        ...tdStyle,
                                                                        color: r.status === 'Pass' ? 'var(--success)' : 'var(--danger)',
                                                                        fontWeight: 600
                                                                    }}>{r.status}</td>
                                                                </tr>
                                                            ))}
                                                    ...
                                                    .map((r, i) => (
                                                        <div key={i} className="mobile-result-row">
                                                            <div className="mobile-result-header">
                                                                <span className="mobile-result-sno">{i + 1}</span>
                                                                <div className="mobile-result-subject">
                                                                    <span className="mobile-result-title">{r.subjectName}</span>
                                                                    <span className="mobile-result-code">{r.subjectCode}</span>
                                                                </div>
                                                            </div>
                                                            <div className="mobile-result-details">
                                                                <div className="mobile-result-stat">
                                                                    <span className="mobile-result-label">CIA/CE</span>
                                                                    <span className="mobile-result-value">{r.internalMarks}/{r.externalMarks}</span>
                                                                </div>
                                                                <div className="mobile-result-stat">
                                                                    <span className="mobile-result-label">Total</span>
                                                                    <span className="mobile-result-value">{r.totalMarks}</span>
                                                                </div>
                                                                <div className="mobile-result-stat">
                                                                    <span className="mobile-result-label">Grade</span>
                                                                    <span style={gradeBadgeStyle(r.grade)}>{r.grade}</span>
                                                                </div>
                                                                <div className="mobile-result-stat">
                                                                    <span className="mobile-result-label">Status</span>
                                                                    <span className={`mobile-result-status ${r.status.toLowerCase()}`}>
                                                                        {r.status}
                                                                    </span>
                                                                </div>
                                                            </div>
